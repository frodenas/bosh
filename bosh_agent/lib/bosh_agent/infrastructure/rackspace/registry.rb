...
module Bosh::Agent
  ##
  # BOSH Agent registry for Infrastructure Rackspace
  #
  class Infrastructure::Rackspace::Registry
    class << self
      ...
      def get_openssh_key
        ...
        return nil unless user_data.has_key?('openssh') && user_data['openssh'].has_key?('public_key')
      ...
      def get_settings
        registry_data = get_json_from_uri("#{get_registry_endpoint}/instances/#{get_server_name}/settings")
      ...
      ##
      # Gets the user data from the injected user data file
      #
      # @return [Hash] User data
      # @raise [Bosh::Agent::LoadSettingsError] if can not get user data
      def get_user_data
        begin
          raw_user_data = File.read(USER_DATA_FILE)
        rescue SystemCallError => e
          raise LoadSettingsError, "Failed to get user data from injected user data file: #{e.inspect}"
        end

        parse_yajl_data(raw_user_data)
      end
      ...
      ##
      # If the BOSH Registry endpoint is specified with a DNS name, i.e. 0.registry.default.rackspace.microbosh,
      ...
      def get_server_name
        ...
        unless user_data.has_key?('server') && user_data['server'].has_key?('name')
          raise LoadSettingsError, 'Cannot get server name from user data'
        end
